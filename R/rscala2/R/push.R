push <- function(what, getType=FALSE) {
  if ( is.integer(what) ) {
    if ( length(what) == 1L ) {
      if ( getType ) TCODE_INT_0
      else writeBin(c(PCODE_PUSH,TCODE_INT_0,what),socketOut,endian="big")
    } else {
      if ( getType ) TCODE_INT_1
      else writeBin(c(PCODE_PUSH,TCODE_INT_1,length(what),what),socketOut,endian="big")
    }
    ...
    if ( length(what) == 1L ) {
      if ( getType ) TCODE_STRING_0
        ...
        writeBin(c(PCODE_PUSH,TCODE_STRING_0,length(bytes)),socketOut,endian="big")
      ...
      if ( getType ) TCODE_STRING_1
        ...
        writeBin(c(PCODE_PUSH,TCODE_STRING_1,length(what),lengths),socketOut,endian="big")
