...
richer API than \rscala.  Like \rJava, \Rserve also requires that \R be
...
binomial coefficient ${10 \choose 3} = \prod_{i=1}^3 (10-i+1)/i$:  
...
The code above uses \Scala's \code{val} statement to define an immutable variable
\code{c_10_3}. The expression \code{(1 to 3)} creates a range. The higher-order \code{map} method of the
range applies the function \code{(10-i+1) / i.toDouble} to each element in the range.
Finally, the \code{map} results are multiplied together by the \code{product} method.

The variable \code{c_10_3} is available in subsequent \Scala expressions as demonstrated
below.
...
in the classpath.  (The period in \code{s\$.} is used to differentiate class names from variables names.)
...
implemented by embedding \Scala code in \R script.  The algorithm is
...
The four implementations of the algorithm are included in the
...
implementation replaces loops with apply functions.  The \Rcpp implementation is
...
destroying the TCP/IP bridge.  The second feature would permit \R and \Scala environments to
