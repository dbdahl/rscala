...
interpreter.  The \code{\%@\%} operator always returns \code{NULL}, e.g.,
...
\code{120} to the console is displayed in the \R session in \emph{many} environments and operating systems.
If the argument ``\code{serialize.output=TRUE}'' is passed to ``\code{scala}''
function when instantiating \Scala, printing is displayed for \emph{all} situations.
This is not the default, however, because the
serializing the output is often not needed yet requires some processing
overhead.

The second operator for evaluating \Scala codes is \code{\%\mytilde\%} which, in
...
Results from previous expressions are available in subsequent expressions, e.g.:

<<evaluateScalaCodeMultiline>>=
s %~% 's"10 choose 3 is $c_10_3"'
@

A \Scala result of class Byte, Int, Double, Boolean, or String are passed to \R
as length-one vectors of raw, integer, double, logical, or character,
respectively.  Likewise, \Scala arrays and rectangular arrays  of these types are passed to \R as
vectors and matrices of the equivalent types.
