...
So far we have demonstrated assessing \Scala from \R.  Conversely, \rscala
can also embed an \R interpreter in a \Scala application via the
...
This assumes that the registry keys option was not disabled during
