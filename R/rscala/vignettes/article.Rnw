...
classes can be instantiated from \R and \Scala methods can be called.
...
implement algorithms in \Scala.  Conversely, \rscala also enables \R code to be
...
snippets in an \R script, accessing precompiled \Scala code from \R, calling
back into \R from embedded \Scala, and developing \R packages based on \Scala.
...
In this section, we demonstrate the easy with which computationally-intensive
statistical procedures can be implemented by embedding \Scala code within an \R
script using the \rscala package.  The algorithm is embarrassingly parallel and
we consider two means of parallelization, one using \Scala's \code{Future}
class and the other using \R's \pkg{parallel} package.  By way of comparison,
we implement the same algorithm using pure \R and using inline \Cpp code via
the \Rcpp package.  All four implementations provide a function taking the same
arguments, including a user-specified sampling function implemented in \R.

The computationally-intensive statistical procedure is a simulation study of
the coverage of a bootstrap confidence interval procedure.  Consider a
population parameter $\beta_1/\beta_2$, where $\beta_1$ and $\beta_2$ are
population quantiles associated with probabilities $p_1$ and $p_2$,
respectively.  Based on a sample of $n$ observations, a point estimator of the
parameter is the ratio of the corresponding sample quantiles and the following
bootstrap procedure can be used when the population distribution is
...
user-supplied function.  Although the code is general, we sample $n=100$
observations from the standard normal distribution and set $p_1=0.75$ and
$p_2=0.35$, making $\beta_1/\beta_2 \approx -1.75$.  We consider
\code{nSamples} = 10,000 and \code{nIntervals} = 10,000.
...
sampling the data.  In the second \rscala implementation, each CPU core has a
separate \R instance with a corresponding \code{RClient}.

The benchmarking results are found in Table \ref{benchmarks}
...
\Scala in one program, allowing a user to leverage their skills in both
languages and to utilized libraries and exploit strengthens in each language.
...
run on separate machines.  The invocation semantics would need to be
generalized, but it is certainly feasible if there is sufficient interest from
the community.
