...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.5.4-SNAPSHOT,\ \ Document ID: 2018-06-04}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
<<scalaScript1qnorm,engine="scala",engine.path=rscala::scalaInfo(s)$cmd,engine.opts=paste0('-nc -cp ',rscala::scalaInfo(s)$classpath),comment="//",eval=(.Platform$OS.type != "windows")>>=
val R = org.ddahl.rscala.RClient()
...
<<scalaScript1europe,engine="scala",engine.path=rscala::scalaInfo(s)$cmd,engine.opts=paste0('-nc -cp ',rscala::scalaInfo(s)$classpath),comment="//",eval=(.Platform$OS.type != "windows")>>=
val R = org.ddahl.rscala.RClient()
