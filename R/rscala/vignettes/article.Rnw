...
Archive Network (CRAN).  Conversely, \R can be embedded within a \Scala
...
software also supports accessing \R from \Scala.  The ability is an offshoot
...
\Rserve users are advised to not ``use Windows unless you really have to''
