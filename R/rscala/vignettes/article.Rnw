...
The consumer of a \Scala function need to know the implementation is written is
\Scala.  There are a few considerations, however, for the programmer of a
\Scala function.  First, the last expression of a \Scala function should always
use either the \code{\%!\%} operator or the \code{\%.!\%} operator.  Second, as
with calling methods of \Scala references, the arguments to \Scala functions
can be copyable types and \Scala references.  Third, default values must be
provided for every argument, thereby indicating to \rscala the type of the
argument.  If the user passes arguments of different types than those indicated
by the default values, they are cast to the expected type.



\subsubsection{Memory Management}

The \rscala package takes advantage of the garbage collection facilities in
both \R and \Scala and, as such, the user often does not need to think about
memory management issues.  There are a few importance things to note, however.

All memory allocated as a result of evaluating a \Scala expression using the \code{\%\mytilde\%} and \code{\%\mytilde\%} operators,
as well as memory allocated by assignment using the \code{\$} operator, cannot be recovered within that instance of \Scala
even if the identifier is later set to another value (e.g., \code{null}).
This is due to
an \href{https://issues.scala-lang.org/browse/SI-4331}{unresolved issue (SI-4331)}
in the \Scala REPL (read-eval-print-loop).
...
on a \Scala instance is


Note that memory allocated as a result of assignment or executing
     `snippet' will never be garbage collected due to a limitation of
     Scala's REPL.  (See the Scala bug detailed here: <URL:
     https://issues.scala-lang.org/browse/SI-4331>.)



The default maximum heap size set by the Java Virtual Machine may not be
sufficient and can be explicitly set in two ways.  It can be set using the
\code{heap.size} argument to the \code{scala} function, or it can be set using
the global option \code{rscala.heap.maximum} (e.g.,
\code{options(rscala.heap.maximum="4G")}).  The former takes presidence over
the latter.
