...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.5.0,\ \ Document ID: 2017-10-12}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
and \code{format}) available in the \Scala snippet (although this can be customized with
the \code{scalaFunctionArgs} function), and ii.~it caches the
...
if ( suppressWarnings(require('microbenchmark', quietly=TRUE)) ) {
  timings <- summary(microbenchmark(fasterNextGaussianRJava(), fasterNextGaussian(),
    rng$nextGaussian(), rngRJava$nextGaussian(), times=1000))
