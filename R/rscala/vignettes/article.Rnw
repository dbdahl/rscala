...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.5.0,\ \ Document ID: 2017-10-10}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
This implicitly makes the interpreter \code{s} available in the current
...
There is no practical difference for \R scripts, but interactive users
