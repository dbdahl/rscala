...
\subsection[Calling Scala Code in R]{Calling \Scala Code in \R}
...
console is displayed in the \R session in \emph{many} environments and
operating systems.  If the argument \code{serialize.output=TRUE} is passed to
the \code{scala} function when instantiating \Scala, printing is displayed for
...
expressions.  For example, we can check that \code{c\_10\_3} (which was defined
in our first \Scala snippet) raised to the power of 2 is in fact equal to $120^2$.
...
calculated \code{Double} to a \code{Double} obtained by parsing a double
literial (i.e., ``6.191736e+20''), which is a slightly different value.
...
as vectors and matrices of the equivalent \R types.  Since the \code{row.major}
argument to the \code{scala} function was left at its default value of
\code{TRUE} when defining the \Scala instance \code{s}, the \Scala code below
results in a 2x5 matrix in \R:
...
\midrule
\noalign{\vspace{2ex}}
\code{z <- as.raw(3)} & \code{e <- as.raw(c(1, 2))} & \code{i <- as.raw(matrix(c(1, 2), nrow=2))} \\
\code{val z = 3.toByte} & \code{val e = Array(1.toByte, 2.toByte)} & \code{val i = Array(Array(1.toByte), Array(2.toByte))} \\
\noalign{\vspace{4ex}}
...
length-one vectors, vectors, and matrices of the equivalent R types.  Each cell
in the table contains two lines: an \R expression (top) and the equivalent
\Scala expression (bottom).  The matrix examples assume that \code{row.major=TRUE} when calling the \code{scala} function.}
\label{mappings}
\end{table}

\subsubsection[Scala References]{\Scala References}

If the result of a \Scala expression is not a primitive type, an array of a
primitive type, or a matrix of a primitive type, then the \code{\%\mytilde\%}
operator will return a \Scala reference to the result which can be used in
subsequent calculations.  If a \Scala reference is desired even working with this
primitive types, use the \code{\%.\mytilde\%} operator, e.g.:




\Scala references are discussed in
more details in Section ??.


(labeled \emph{primitives}), as well as vectors and
matrices of these types can be seamlessly copied between \R and \Scala.
