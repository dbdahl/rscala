...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.3.4,\ \ Document ID: 2017-08-29}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
<<sbt, engine="scala",eval=FALSE,highlight=(.Platform$OS.type != "windows")>>=
libraryDependencies += "org.ddahl" %% "rscala" % "2.3.4"
@
...
<<scalaScript0,engine="scala",eval=FALSE,highlight=(.Platform$OS.type != "windows")>>=
...
<<scalaScript1qnorm,engine="scala",engine.path=scalaInfo(s)$cmd,engine.opts=paste0('-nc -cp ',scalaInfo(s)$classpath),comment="//",eval=(.Platform$OS.type != "windows"),highlight=(.Platform$OS.type != "windows")>>=
val R = org.ddahl.rscala.RClient()
...
<<scalaScript1europe,engine="scala",engine.path=scalaInfo(s)$cmd,engine.opts=paste0('-nc -cp ',scalaInfo(s)$classpath),comment="//",eval=(.Platform$OS.type != "windows"),highlight=(.Platform$OS.type != "windows")>>=
val R = org.ddahl.rscala.RClient()
