...
\subsubsection[Calling Scala methods]{Calling \Scala methods}
...
\subsubsection[Discovering Scala Methods]{Discovering \Scala Methods}
...
\Scala objects can be instantiated
in three ways.
New instances of the \code{scala.util.Random}
class, for example, can be created in the following three ways:
...
self-evident to a \Scala developer but is the slowest.
...
Again, as for instantiating objects, each way has its advantages in terms of
flexibility, readability, and speed.

\subsubsection[Method arguments, null, and length-one vectors]{Method arguments, \code{null}, and length-one vectors}

Arguments to \Scala methods called using the \code{\$} operator on a \Scala
reference can be copyable types and \Scala references.  To pass a null reference
of a particular type, use the \code{scalaNull} function.  For example,
to clear the system properties, a null reference to \code{java.util.Properties} is passed to the static \code{setProperties}
method of the \code{java.lang.System} class.

<<nullAsArgument>>=
s$.java.lang.System$setProperties(scalaNull('java.util.Properties'))
@

Length-one vectors in
