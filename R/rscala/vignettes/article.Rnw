...
can also be a literal (as in this example) or a variable.  Note that \code{do} is used here because it is a
...
reference can be copyable types and \Scala references.  To pass a null
reference of a particular type, use the \code{scalaNull} function.  For
example, the documentation for \Java's \code{java.lang.System} class says that
a null reference to \code{java.util.Properties} is passed to the static
\code{setProperties} method to clear the system properties.  This is
...
When calling the \code{apply} method of the \code{setter} object, the first argument must be an array.
...
`\code{s\$do(}' notations --- which were first introduced for object instantiation and calling methods of singleton
