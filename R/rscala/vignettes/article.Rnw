...
(SBT).  The \rscala software has no dependencies beyond \Scala's standard library
and base \R.
...
typically implemented in \C, \Cpp, and \Fortran code.  The \Rcpp \citep{rcpp} package
makes it easy to integrate \C and \Cpp code in \R.
The \rJava \citep{rJava} package adds \Java as a fourth language for writing \R
...
made available to the broader \R community using \rscala.

Beyond writing \Scala-based packages, \rscala also provides a simple and
convenient way to leverage \Scala in an existing \R workflow.  For example,
arbitrary \Scala snippets can be included within an \R script and inline \Scala
functions allow the user to implement \R functions using \Scala.  Also, \rscala
supports callbacks to \R from \Scala, which enables \Scala implementations of
general algorithms (e.g., root finding methods) based on user-supplied \R
functions.  In short, \Scala and \R each have distinct strengths and our
\rscala software allows the user to easily leverage the advantages of each in a
single project.
...
is appealing from a users' perspective.  Finally, callbacks in \rJava are
provided by the optional \pkg{JRI} component which is only available if \R is
...
software if it were based on \rJava's \pkg{JRI}.
