...
This makes the interpreter \code{s} available in the current
...
There is no practical difference for \R scripts, but interative users
may notice that \code{scala} feels faster.  Details on this and all
