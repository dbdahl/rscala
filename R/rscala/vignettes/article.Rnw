...
Archive Network (CRAN).  Furthermore, \R can be embedded within a \Scala
application by adding a one-line dependency declaration in \pkg{Scala Build Tool}
(SBT). ///Replaced Conversely with Furthermore because the former has already been used in the same paragraph.

///All of the original ideas in this paragraph have been preserved; I just moved them around to be in an order closer to 1) claim; 2) evidence supporting the claim; and 3) conclusion. I suggest removing the phrase about Java bytecode. The significance of compiling to Java bytecode isn't clear without further explanation, in my opinion.

\Scala is a general-purpose programming language that is designed to strike a
balance between execution speed and programmer productivity.  \Scala programs run on
the Java Virtual Machine (JVM) at speeds comparable to \Java.  \Scala features
object-oriented, functional, and imperative programming paradigms, affording
developers flexibility in application design.  \Scala code can be concise,
thanks in part to type inference, higher-order functions, multiple inheritance
through traits, and a large collection of libraries.  \Scala also supports
pattern matching, operator overloading, optional and named parameters, and
string interpolation. \Scala encourages immutable data types and pure
functions (i.e., functions without side-effects) to simplify parallel
processing and unit testing. In short, the \Scala language implements many of the most
productive ideas in modern computing. To learn more about \Scala,
we suggest \emph{Programming in Scala} \citep{scalabook} as an excellent
general reference.

Because \Scala is flexible, concise, and quick to execute, it is emerging as an important tool for scientific computing. For example, 
\pkg{Spark} \citep{spark}, the
cluster-computing framework for massive datasets, is written in \Scala.
...
syntatical differences.  \R has a massive user base of statisticians and over
11,000 actively-maintained packages on CRAN. Hence, the \Scala community has a lot to gain from 
an integration with \R.

/// I struggled with the following commented paragraphs because the focus shifts throughout and some of the ideas could be grouped together. I provide a different version, but some of the sentences need to be finished. They are marked with `///'. Also, I prefer to leave out the shallot and bamboo references. The sentences are complicated to include, and they are referenced again later in the paper.

% \R code can be very concise and expressive, but may run significantly slower
% than compiled languages.  In fact, computationally intensive algorithms in \R are
...
% extensions.  The \rscala software adds \Scala as another language for
...
% \citep{shallotPaper,bambooPaper} that were originally developed in \Scala, but
...
% convenient way to leverage \Scala in an existing \R work flow.  For example,
...
are typically implemented in compiled languages such as \C, \Cpp, \Fortran,
and \Java. The \rscala package adds \Scala to this list of high-performance
...
integration may not be as comprehensive in scope as \Rcpp and \rJava, it
provides the following important features to blend \R and \Scala.

\begin{itemize}

  \item \rscala allows arbitrary \Scala snippets to be included within an \R script and
\Scala objects can be created and referenced directly within \R code. These
features allow developers to use \Scala solutions in an existing \R workflow.

  \item \rscala also supports callbacks to \R from \Scala. Callbacks allow developers to implement general, high-performance, algorithms in \Scala (e.g., root finding methods), that can
invoke user-supplied \R functions.

  \item \rscala provides a special interface to simplify developing \R packages based on \Scala solutions. ///more details///
  
  \item RClient stuff///
  
\end{itemize}

In essence, \rscala's feature-set makes it easy to 
exploit the strengths of \R and \Scala in a single project.

///Need a segue sentence. Complete shift of topic here. Consider some preamble about application architecture and existing work.
...
/// This opening sentence jars me. We are still talking about application architecture. I would use a sentence that indicates that we are still talking about arhitecture.

The discussion so far has focused on accessing \Scala from \R.  The \rscala
software also supports accessing \R from \Scala.  This ability is an offshoot of
the callback functionality.  Since \Scala can call \Java libraries, those who
are interested in accessing \R from \Scala should also consider the \Java
libraries \Rserve \citep{rserve} and \pkg{RCaller} \citep{rcaller}.  Like
\rscala, \Rserve is ``a TCP/IP server which allows other programs to use
facilities of \R'' (\url{http://www.rforge.net/Rserve}).  \Rserve clients are
available for many languages including \Java.  \Rserve is fast and provides a
much richer API than \rscala.  Like \rJava, \Rserve also requires that \R be
compiled as a shared library.  Also, Windows has some limitations such that
\Rserve users are advised not to ``use Windows unless you really have to''
...
function taking the same arguments, including a user-specified \R function for
sampling.
...
used when the population distribution is unspecified.  The sample estimate is
...
\rscala implementation (which uses the \pkg{parallel} package) was the faster
overall.  The first \rscala implementation was close behind, expect on the
...
relative to the pure \R implementation.
