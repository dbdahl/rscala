...
programmer of a \Scala function.  First, the final expression of a \Scala
...
indicate arbitrary \R objects that will not be converted to \Scala values but will still be used in \R code.  Third, a default value must be provided for
...
rng2 <- rJava::.jnew("scala.util.Random")
.jcallNextGaussian <- function() rJava::.jcall(rng2, "D", "nextGaussian")
...
Since \Scala is binary-compatible only within major versions, package developers
are encouraged to cross-compile for versions 2.10, 2.11, and 2.12 and to place
