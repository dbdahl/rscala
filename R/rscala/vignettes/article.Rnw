...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.2.3,\ \ Document Date: 2017-07-24}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
This section provides a guide to access
...
The \rscala package requires a \Scala installation of the 2.10.x, 2.11.x, or
2.12.x series.  A convenience function,
\code{rscala::scalaInstall()}, is provided to download and install \Scala in the user's
home directory under \code{.rscala}. Because this is a user-level
...
By default, the \code{scala} function searches the system for a suitable
\Scala installation to create the instance.  Use the \code{scalaInfo} function
as follows to display the default \Scala installation and \rscala's search
sequence.
...
The \rscala bridge is not thread-safe, so multiple \R processes/threads should not access the same \Scala
...
\subsubsection[Evaluation of Scala snippets]{Evaluation of \Scala snippets}

\Scala code can be compiled and executed within an \R session using several
operators.  The most basic operator is \code{\%@\%}, which evaluates a snippet of \Scala code and returns \code{NULL}.
...
This example defines a range and calls \code{foldLeft}, a higher-order function 
(i.e., a function that takes another function as a parameter),
to compute the product of computations from the supplied function literal.
The result is available in subsequent \Scala expressions by the identifier \code{c_10_3}.

\Scala code can also be evaluated with the \code{\%\mytilde\%}
operator which returns to \R the result of the last expression in the \Scala snippet.

<<evaluateScalaCodeSimple2>>=
tenChooseThree <- s %~% '(1 to 3).foldLeft(1) { (prod,i) => prod * (10-i+1)/i }'
tenChooseThree
@

\subsubsection{Console output}

\Scala can display console output as demonstrated below.
...
function.  Default values are based on the operating system such that
console output is displayed in a typical environments, but this behavior can be modified using these arguments.
...
s %@% '
  val c_10_3 = (1 to @{k}).foldLeft(1) { (prod,i) => prod * (@{n}-i+1)/i }
  println("The @{label} is " + c_10_3 + ".")
'
@
...
example, that the second expression below is false. This is because \code{@\{tenChooseThree\^{}20\}} is replaced by a string literal
(i.e., \code{6.191736e+20}, which is slightly different than the calculated value.

<<evaluateScalaCodeInterpolationError>>=
s %~% 'math.pow(c_10_3, 20) == @{tenChooseThree^20}'
@
...
references can be used to call methods directly using the \code{\$}
...
\code{recursive.sum} function in \R below repeatedly calls itself from \Scala to
