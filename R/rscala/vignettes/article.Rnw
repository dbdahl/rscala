...
This paper introduces \rscala, software providing a bridge between \R and
\Scala.  The goal of the software is to allow users to easily leverage the
unique strengths of \Scala and \R to make them more productive.  \Scala can be
embedded within \R using the \rscala package available on the Comprehensive R
Archive Network (CRAN).  Callbacks from embedded \Scala into \R are supported.
\R packages can implement computationally-intensive algorithms in \Scala.
Conversely, \R can be embedded within a \Scala application by a one-line
dependency declaration in \pkg{Scala Build Tool} (SBT).
...
paradigms.  The integration of these two paradigms allows a developer to choose
the best paradigm for a given task.  Code is generally concise, thanks in part
to type inference, higher-order functions, and a large collection of libraries.
\Scala source code compiles to \Java bytecode and runs on the Java Virtual
Machine (JVM) at speeds comparable to \Java.  As such, \Scala strikes a
pleasant compromise between execution speed and programmer productivity.
\Scala is the language in which \pkg{Spark}, a cluster-computing framework for
massive datasets, is written.  More generally, a number of books have recently
published on using \Scala for data science \citep{a9781785281372}, scientific
computing \citep{a9781785886942}, machine learning \citep{a9781783558742}, and
probabilistic programmings \citep{a9781617292330}.  We believe that \Scala
...
The \rscala software has no dependences beyond the \Scala's standard library
and base \R.  The bridge between \R and \Scala is based on a custom TCP/IP
protocol.  This affords flexibility but it is natural to question whether
\rscala could have been developed more easily by building on existing software.
Since \Scala code compiles to \Java byte code and runs on the JVM, one could
access \Scala from \R via \rJava and then benefit for the speed of shared
memory.  In fact, an early implementation of (what is now known as) \rscala was
indeed based on \rJava.  This implementation was abandoned for several reasons.
First, some users struggle with installing and maintaining a working \rJava
installation.  The \rJava issue tracker on GitHub
(\url{https://github.com/s-u/rJava/issues}) shows a number of closed issues
that users have with \rJava maintenance.  In fairness to \rJava, these are
mostly user errors.  Nevertheless, the reach of package based on \rJava may be
limited because some users lack the skills or patients to resolve technical
issues regarding system maintenance.  Second, \rJava and \Scala's
read-eval-print loop (REPL) are both implemented using custom class loaders
which seem to conflict with each other.  Third, since \rJava links to a single
instance of the JVM, there is the possibility that an \rJava-based package
configures the JVM in a manner that is not compatible with a second
\rJava-based package.  Finally, callbacks in \rJava are provided by the
...
callback functionality and came at little marginal cost.  Since \Scala can call
\Java libraries, those who are interested in accessing \R from \Scala should
also consider the following \Java libraries for accessing \R: \Rserve
\citep{rserve} and \pkg{RCaller} \citep{rcaller}.  Like \rscala, \Rserve ``a
...
The paper is organized as follows.  Section \ref{scalainr} describes \rscala's
functionality to access \Scala from \R.  Some of the more important topics
...
Section \ref{rinscala} describes \rscala's functionality to access \R from
\Scala.  Throughout these sections, concise examples are provided to help
describe the functionality and illustrate why one might interested in bridging
\R and \Scala.  Section \ref{bootstrap} provides an case study to show how
\Scala can easily be embedded in \R to significantly improved the computation
...
Section \ref{bootstrap} considers the easy and execution speed of implementing
a simulation study in \R, \Cpp via \Rcpp, and \Scala via \rscala.  It is not a
comprehensive comparison of the speed of these languages.  For that, we refer
