...
Note that the \rscala package does \emph{not} even need to be installed in \R.  The
example below shows a simply \Scala snippet that: 1.~finds the city in Europe that is
closest on average to all other European cites (as given by \R's builtin dataset \code{eurodist}), and
ii.~wraps \R's \code{qnorm} function to define a method in \Scala by the same name:
...
The previous example assume that, on Windows, the registry keys option was
