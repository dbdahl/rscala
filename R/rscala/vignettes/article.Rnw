...
flexible but slightly awkward.  It has the added advantage, however, that the
...
We now consider execution speed.  We will not attempt to benchmark \Scala
itself \textit{vs}. other languages.  For that, we refer readers to various
benchmarks available on the web, including \citet{languageShootout}.  In short,
\Scala seems to perform at speeds comparable to \Java, and \Java is found to be
1.5 to 3 times slower than \C.

Here we wish to highlight some of performance characteristics associated with
the \rscala package.  Re
...
application is hosted here: \begin{center}
\url{https://dahl.byu.edu/software/rscala/temperature/} \end{center} and the
source code is available here: \begin{center}
\url{https://github.com/dbdahl/rscala-example-temperature/} \end{center} Figure
\ref{screenshot} shows a screen shot this application.
