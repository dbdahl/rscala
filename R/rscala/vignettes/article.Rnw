...
array of statistical libraries in \R.  Callbacks from embedded \Scala into \R
...
library and base \R.  Conversely, \R can be embedded within a \Scala
application by a one-line dependency declaration in \pkg{Scala Build Tool}
(SBT).
...
paradigms, which provide flexibility to accomplish a given task.  Many books on
\Scala are available to particular audiences, but an excellent general
...
functions without side-effect), concurrrent processing on multicore processors
and unit testing can be simplified.  \Scala is designed to strike a balance
between execution speed and programmer productivity.  Source code compiles to
\Java bytecode and runs on the Java Virtual Machine (JVM) at speeds comparable
to \Java.  \Scala is the language in which \pkg{Spark} \citep{spark}, a
...
libraries are in their relative infancy, \R's has over 11,000
actively-maintained packages on CRAN.
...
methods.  The \rJava package adds \Java as a fourth language for writing \R
extensions.  The \rscala software seeks to add \Scala as another language for
writing computationally-intensive algorithms accessible from \R.  Roughly,
\rscala is to \Scala, as \rJava is to \Java and as \Rcpp is the \C and \Cpp.
The level of integration between \Scala and \R provided by \rscala is not as
deep as those available to \C, \Cpp, \Fortran, and \Java.  Still, rscala is
sufficient in many cases to allow \Scala developers to make their software
available in \R and thus to a much wider audience.  For example, \rscala
supports callbacks to \R from \Scala, which enables \Scala implementations of
general algorithms (e.g., root finding methods) based on user-supplied \R
functions.  The \pkg{shallot} \citep{shallotSoftware} and \pkg{bamboo}
\citep{bambooSoftware} packages on CRAN are two examples of software
implementing statistical methodology of associated journal articles
\citep{shallotPaper,bambooPaper} that was originally developed in \Scala, but
made available to the broader \R community using \rscala.  Beyond writing
\Scala-based packages, \rscala also provides a simple and convenient way to
leverage \Scala in an existing \R workflow.  In short, \Scala and \R each have
distinct strengths and our \rscala software allows the user to easily leverage
the advantages of each in a single project.
