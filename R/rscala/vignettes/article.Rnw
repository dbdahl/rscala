...
\Scala \citep{scala} is a general-purpose, statically-typed programming language that can be used for enterprise-level
solutions such as \pkg{Spark} (CITATION), a popular cluster-computing framework for
massive datasets.
