...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.3.1,\ \ Document Date: 2017-08-14}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
identifier name.  For example, the following both provide equivelent
...
vectors have special symantics.  In the \R expression \code{rng\$nextInt(10L)},
...
For example, the \pkg{shallot} \citep{shallotSoftware} and \pkg{bamboo}
\citep{bambooSoftware} packages on CRAN use \Scala via \rscala to implement
statistical methodology of their associated journal articles
...
\url{https://github.com/dbdahl/rscala/}.  As a convenience, \rscala's JAR file
is available in standard repositories for use by dependency management systems.
To include \code{org.ddahl.rscala.RClient} in a \Scala application, simply add
the following line to SBT's \code{build.sbt} file.
...
and \Python.  \R users who are not already familar with \Scala would be best
served by accessing \pkg{Spark} through a dedicated \R package such as
\pkg{sparklyr} or \pkg{sparkr}.  \Scala developers, however, might prefer to
interface directly with \pkg{Spark}'s Machine Learning Library (MLlib) by a
\Scala application supplemented with \R.  Although we do not have firsthand
experience with this, we have provided support to a developer using \rscala in
\pkg{Spark}.  Recall that every \code{RClient} has its own workspace, so
several instances should be used to overcome the single-threaded nature of \R.
Specifically, the developer is using \pkg{Apache Commons Pool} to manage a pool
of \code{RClient} objects on each \pkg{Spark} worker node.
