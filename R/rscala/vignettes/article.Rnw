...
published on using \Scala for data science, machine learning, and statistical
computing.  (Citations) We believe that \Scala deserves consideration when
looking for a general-purpose programming language to complement \R.
