...
the \pkg{shallot} and \pkg{bamboo} packages on CRAN implement statistical
methodology of associated journal articles using \Scala accessed via the
\rscala package.  The readers are invited study those examples, but we highlight a few items here.
An \R package based on \rscala should include \code{rscala} in the
\code{Imports} field of the package's \code{DESCRIPTION}
file and the \code{NAMESPACE} file should include a line with \code{import(rscala)}.
The package should define an
\code{.onLoad} function which calls \code{.rscalaPackage(pkgname)}, where
\code{pkgname} is the package's name, e.g.:
...
Analogous to \rJava's \code{.jpackage} function, the \code{.rscalaPackage}
function adds the files in the source package's \code{inst/java} directory to
\Scala's classpath.  \code{.rscalaPackage} function takes several optional
arguments.  The \code{classpath.packages} allows a package to use the JAR files
of another package.  For example, the \pkg{shallot} packages imports the
\pkg{commonsMath} package and has \code{classpath.packages="commonsMath"} so
that the \pkg{shallot} package does not need to distribute itself the JAR file
of the Apache Commons Mathematics Library.


 that: i.~provide additional functionality to set the classpath, ii.~give a \Scala snippet to ev

     the JAR files in the ‘java’ directory of the package and passes
     the ‘...’ arguments to the ‘scala’ function.  This instance of
     Scala is available as the object ‘s’ in the namespace of the
     package (thereby making it available to the package's function)
     but it is not exported from the namespace.  The object ‘s’ is only
     initialized on its first usage. The function
     ‘.rscalaPackageUnload’ should be called in the ‘.onUnload’
     function of a package that wishes to depend on this package so
     that ‘close(s)’ is called (if needed).



Since the namespace is not yet sealed,
