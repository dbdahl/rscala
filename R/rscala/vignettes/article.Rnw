...
order, the TCP/IP ports for the local socket connections, and how output from
...
history, etc.  Multiple \R processes/threads should not access the same \Scala
instance simultaneously, but \Scala itself can use multiple threads/cores.
...
\Scala code that makes use of these features from \rJava requires some
understanding of \Scala's compiler at a minimum, but often the developer is
