...
Archive Network (CRAN).  Also, \R can be embedded within a \Scala
...
\pkg{Spark} \citep{spark} is a
cluster-computing framework for massive datasets written in \Scala.
...
integration is not as comprehensive as \Rcpp and \rJava, it
provides the following important features to blend \R and \Scala.  First,
\rscala allows arbitrary \Scala snippets to be included within an \R script and
\Scala objects can be created and referenced directly within \R code. These
features allow users to integrate \Scala solutions in an existing \R workflow.
Second, \rscala supports callbacks to \R from \Scala, which allows developers to implement general, high-performance algorithms in \Scala (e.g., root finding methods) based on user-supplied \R functions.
Third, \rscala supports developing \R packages based on \Scala which allows \Scala developers to make
their work available to the \R community.  Finally, the \rscala software makes it easy to incoporate \R in a \Scala application
without even having to install the \R package.
In sum, \rscala's feature-set makes it easy to 
...
The discussion of the design of \rscala has so far focused on accessing \Scala from \R.  The \rscala
software also supports accessing \R from \Scala using the same TCP/IP protocol.  This ability is an offshoot of
...
much richer API than \rscala.  Like \rJava, however, \Rserve also requires that \R be
