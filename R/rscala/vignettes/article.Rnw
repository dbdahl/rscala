...
This section provides a comprehensive overview of \rscala's ability to access
...
The \rscala package requires a \Scala installation in the 2.10.x, 2.11.x, and
2.12.x series.  System administrators can install \Scala using their operating
...
\Scala webpage (e.g., download the archive, unpack it, and add the \code{scala}
script to the path).  \Scala itself requires that \Java is installed.  The
\rscala package provides various methods to find a suitable \Scala
installation, including searching the path.  Details on the search are
available using:
...
One of places that \rscala looks for \Scala is under user's directory
\code{\mytilde/.rscala} and it can be installed there using:
...
customize: i.~how \Scala is found and the desired \Scala version, ii.~how it is invoked (e.g., setting the classpath
and maximum heap size), iii.~whether matrices are in row-major or column-major
order, iv.~the TCP/IP ports for the local socket connections, and v.~how output from
\Scala is treated.  Details on this and all other functions are provided in the
\R documentation for the package (e.g., \code{help(scala)}).  A \Scala session
...
history, etc.  For a given \Scala instance, the \rscala bridge is not thread
safe so multiple \R processes/threads should not access the same \Scala
instance simultaneously, nor should multiple \Scala threads call back to \R
simultaneously.
...
snippet of \Scala code that is evaluated in the embedded \Scala instance.  The
\code{\%@\%} operator always returns \code{NULL}.  Consider, for example,
...
console is displayed in the \R session.  Output behavior may depend on the
environment and operating system in which R is run and the arguments
\code{serialize.output}, \code{stdout}, and \code{stderr} of the \code{scala}
function can be used to customize console output.

\subsubsection{String Interpolation}

The \rscala package provides for string interpolation to evaluate arbitrary \R
code and replace its string representation before evaluating the entire string
as \Scala code.  The right-hand-side character vector is scanned for
``\code{@\{}'', followed by an arbitrary string, and ended by ``\code{\}}''.
The arbitrary string is evaluated as an \R expression in the environment in
which the \code{\%@\%} operator is called.  Finally, \code{@\{...\}} is
replaced with the string representation of the \R expression and the snippet is
evaluated as \Scala code.  For example,
...
Another interface to the Scala instance is provided by the \code{\%\mytilde\%}
operator which, in addition to evaluating the \Scala snippet, returns the
result of the last expression to \R, e.g.,
...
expressions.  For example, we can check that the \R expression
\code{nThreesomes}\verb=^=2 is in fact equal to \code{c\_10\_3} (which was
defined in our first \Scala snippet) raised to the power of 2.
...
\code{Double} obtained by parsing a literal obtained by string interpolation
(i.e., ``6.191736e+20'') to a calculated \code{Double}, which is a slightly
different value.
...
arrays of arrays of the primitive types.  The name is meant to remind us that
passing this kind of data between \Scala and \R involves serializing the data
over the TCP/IP interface, a potentially-costly exercise for a large data
structures.  Since the \code{row.major} argument to the \code{scala} function
was left at its default value of \code{TRUE} when defining the \Scala instance
\code{s}, the code below results in a 2x5 matrix in \R:
...
example snippets and assume the \code{scala} function was called with argument
\code{row.major=TRUE}.  Variables with the same identifier (e.g., \code{b}) are
equivalent in \R and \Scala.  For example, a vector of integers in \R (e.g.,
\code{h <- c(1L, 2L, 3L)}) maps to the \Scala type \code{Array[Int]} (e.g.,
\code{val h = Array(1, 2, 3)}).
...
Taking inspiration from \rJava's ``high-level \code{\$} operator,'' \Scala
references can also be used to call methods more directly using the \code{\$}
operator for \Scala references.  For example, the previous example can be
...
reference can be copyable types and \Scala references.  Length-one vectors in
\R have special handling.  In the previous \R expression
\code{rng\$setSeed(24234L)}, the value \code{24234L} is an integer vector of
length one in \R, but is passed to \Scala as \code{Int}, not \code{Array[Int]}.
This behavior is usually convenient and natural behavior.  If, however, an \R
vector should be treated as an array by \Scala, despite the fact that it might
be of length one, wrap the vector in a call to the \code{I} function to ensure
that the vector is treated ``as is''.  For example, consider an object with an
\code{apply} method that takes an array and a value and sets every element of
...
Thus, if there is a potential that the \R vector is length-one, it should be
wrapped by the \code{I} function.  In the example below, the first argument is
wrapped by \code{I}, but the second argument is a length-one double vector in
\R treated as a \code{Double} (instead of \code{Array[Double]}) because it is
\emph{not} wrapped by \code{I}:
...
Methods of companions objects --- which are roughly \Scala's equivalent to
\Java's static methods --- can be called in three ways.  For example, consider
the companion object \code{Array}.  Its \code{range} method creates an array of
...
These three equivalent expression each have their advantages in terms of
...
can be variables or literals.  The last is also fast, somewhat awkward, but has
the added flexibility that the name of the companion object itself (as well as
the arguments) can be variables or literals.
...
\code{\%.\mytilde\%} operator.  Likewise, to obtain a \Scala reference for the
other two expressions in the previous example, a final argument
\code{.AS.REFERENCE=TRUE} should be used.  These are illustrated below in the
following three functionally equivalent expressions:
...
compiler when appropriate, but must be explicit called when using the
\code{\$} operator.
...
We can assign the value of $\pi$ to the second element of the array using the
following functionally equivalent approaches:
...
``\code{s\$do("}'' notations --- which were first introduced for companion
objects --- can also be used for existing \Scala values (e.g., \code{fibSeq},
as shown above).
...
\Scala has a type parameterization which are similar but arguably more
...
its type parameter may need to quoted to prevent parsing errors in \R.  The
...
\Scala values can also be obtained using the \code{\$} operator and the name of
the identifier.  For example, the following both return the same result to the
first example (involving \code{s \%\mytilde\%  'fibSeq'}):
...
The second example (involving \code{s \%.\mytilde\% 'fibSeq'}) yields the same
result as:
...
Likewise, values of copyable types and \Scala references may be set in the
...
Or, if you already have a \Scala reference (e.g., the previously-defined
\code{rng}), you can simply use the \code{scalap} function as follows:
...
evaluation in the \R session.  This feature is inspired by the packages
\pkg{inline} and \Rcpp packages in \R for \C, \Cpp, and \Fortran.  We do not
...
writing code to interface with existing \Scala code.

To demonstrate \Scala functions, consider computing the Bell number
\citep{bell} (i.e., the number of partitions of $n$ items) for finite mixture
models and random partition models (e.g., \citet{casella2014}).  First,
consider an \R implementation of an computationally- and memory-efficient
algorithm for computing the Bell number based on the Bell triangle (also known
as the Aitken's array or the Peirce triangle):
...
for $n \le 22$, provides \emph{approximate} answers for $22 < n \le 218$, and
...
arrays, there are syntactically differences between \R and \Scala, and the
\Scala version avoid the copying found in the \R version), but the practical
...
$n$, but there is no overflow for practically any $n$ when \code{format="log"}
and the exact value is provided when \code{format="character"}.  Consider the
\Scala function below that demonstrates: i.~exact integer calculations for
large $n$, and ii.~that subsequent calls to \Scala functions are faster than
the first call because of caching of the complication of the \Scala snippet:
...
The consumer of a \Scala function may not need to know that the implementation
is written is \Scala.  There are a few considerations, however, for the
programmer of a \Scala function.  First, the last expression of a \Scala
function should always use either the \code{\%!\%} operator or the
\code{\%.!\%} operator.  Second, as the arguments to \Scala functions can be
copyable types, \Scala references, or \code{NULL}, where \code{NULL} is used to
indicate arbitrary \R objects.   Third, default values must be provided for
every argument, thereby indicating to \rscala the type of the argument.  If the
user passes arguments of different types than those indicated by the default
values, they are cast to the expected type.  The \code{scalaNull} function can
be used to specify a \Scala reference  without having to instantiate an object
of that class.  For example, an argument to \Scala function might be \code{rng
= scalaNull("scala.util.Random")}, indicating that the argument should be an
object of class \code{scala.util.Random} and eliminating the need to have such
an object when the \Scala function is defined.
...
\code{R}.  This object provides access to the \R session from the \Scala
instance.  The Scaladoc for the \code{RClient} class, which is linked on the
landing page of the the \rscala's GibHub repository
(\url{https://github.com/dbdahl/rscala}), provides comprehensive documentation
for the methods.  Here we demonstrate the functionality.  To assign a value in
...
whose static type is \code{Any} and a \code{String} representing its more
specific type according to the \Scala compiler.  Consider the example below in
...
\R expressions can also be evaluated with a suite of method whose name starts
with \code{eval} and ends in $XY$, with $X$ and $Y$ having the same meaning as
the methods starting with \code{get}.  In the example below, \R and \Scala are
used together to sample from chi-square distribution with $100$ degrees of
freedom:
...
$\alpha$ can be obtained using root finding methods (e.g., the \code{uniroot}
function in \R).  Here, we demonstrate the bisection method implemented as a
\Scala function and using callbacks to access an anonymous, user-supplied \R
function:
...
conversion) and a value \code{x}.  The \code{invokeD0} call the \code{R}
function and returns the result as a \code{Double}.
...
There are a few important things to note, however.  First, the default maximum
heap size (as set by the Java Virtual Machine) may not be sufficient and can be
...
applications.  Further, as will be shown later, these also enjoy less
invocation latency than the equivalent code using the \code{\%\mytilde\%} and
\code{\%.\mytilde\%} operators.
...
and we describe the process below.  An \R package based on \rscala should
...
are encouraged to support major version 2.10, 2.11, and 2.12 by placing
...
a package only supports specific major versions of \Scala, the
\code{major.version} argument should be overridden when calling the
\code{.rscalaPackage} function.

The \code{.rscalaPackage} function takes several optional arguments.  The
\code{classpath.packages} allows the package to use the JAR files of another
package.  For example, the \pkg{shallot} package imports the \pkg{commonsMath}
package \citep{commonsMathSoftware} and uses the \code{classpath.packages}
argument so that the \pkg{shallot} package can use the Apache Commons
Mathematics Library without having to bundle its JAR file.  (The rational is
that library JAR files can be large and, by having them in a separate package,
they do not need to be updated as frequently and other packages can make use of
them too.) The arguments \code{classpath.prepend} and \code{classpath.append}
give fine-grained control over the classpath.  The argument \code{snippet}
provides \Scala expressions that should be evaluated once \Scala is eventually
instantiated.  This is a useful to provide, for example, definitions and import
...
based on the \rscala package can be very familiar to those accustomed to the
\rJava package.  Take, for example, CRAN's \pkg{mailR} package: ``Interface to
Apache Commons Email to send emails from \R'' \citep{mailr}.  This package
depends on the \rJava package and mostly uses its high-level \code{\$}
...
our port is at \url{https://github.com/dbdahl/mailR}.  The port involved two
script files, the \code{DESCRIPTION} file, and the \code{NAMESPACE} file.  We
deleted 16 lines, added 4 lines, and modified 15 lines.  Most modifications
were simple changes, like:
...
\code{.jcall} function) will require more changes.  The 
...
This example also illustrates the difficulty in calling \Scala from \rJava
since \Scala provides several features that do not map directly to \Java
equivalents.  The \Scala compiler uses name mangling, behind-the-scenes code
generation, and other techniques when compiling to \Java bytecode.  Calling
\Scala code that makes use of these features from \rJava requires at a minimum some
understanding of \Scala's compiler, but often the developer is
...
for callbacks into \R from embedded \Scala, but it can be useful in its own
right.
...
and uses a standard installation of \R.  As a convenience, \rscala's JAR are
available in standard repositories for use by standard dependency management
systems.  To make the \code{org.ddahl.rscala.RClient} class available to a
...
This assumes assume that the registry keys option was not disabled during
...
explicitly (e.g., \code{org.ddahl.rscala.RClient("/path/to/R/bin/R")}).

We have found that embedding \R can be an easy and convenient way to facilitate
statistical calculations, access statistical functions, manage data, and
produce plots in a \Scala application.  All four of these are illustrated in
the following example which: 1.~finds the city in Europe that is closest on
average to all other European cites (as given by \R's builtin dataset
\code{eurodist}), ii.~wraps \R's \code{qnorm} function to define a method in
\Scala by the same name, iii.~write data to disk and reads it again, and
iv.~produces a dendrogram for hierarchical clustering:
...
// Item 3: Read and write data to disk
...
The \code{scala} function instantiates a \Scala interpreter using R's
\code{system2} function to asynchronously run \Scala code which implements a
TCP/IP server whose port is randomly selected by default, but can be fixed at a
given value.  The server accepts only a single local connection.  While the
server is starting up, \R repeatedly tries to connect to it as a client using
functions from the \pkg{base} \R package.  The \Scala server has an instance of
\code{scala.tools.nsc.interpreter.IMain}, the class representing \Scala's
read-eval-print loop (REPL).  The \R client accesses the functionality of the
server's REPL class by means of a custom protocol.

The \rscala package takes advantage of \Scala's REPL ability to compile and
execute \Scala code on the fly as well as setting and getting values of
identifiers.  When a \Scala function is called for the first time (or when a
particular method of a \Scala reference is called for the first time), a
one-time compilation occurs behind the scenes.  The compiled method is then
...
\Scala version (\code{org.ddahl.rscala.RClient}).  The wrapper provides
familiar Javadoc to \Java developers and also hides various \Scala-specific
features.
