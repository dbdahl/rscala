...
snippet of valid Scala code (containing any mixture of statements, functions,
class definitions, imports, etc.) and does \emph{not} provide any results to
\R, e.g.:

<<evaluateScalaCodeSimple>>=
s %@% 'val sqrt2 = math.sqrt(2)'
@
