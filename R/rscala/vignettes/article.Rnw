...
allows for the definition of inline \Scala functions within an \R session.
These methods are compiled on the fly and cached for execution later in the \R
session.  We demonstrate this functionality by defining a function to compute
the number of partitions of $n$ items, i.e., the Bell number \citep{bell}:
...
Note that the object \code{bell.using.rscala} is an \R function which calls the associated
