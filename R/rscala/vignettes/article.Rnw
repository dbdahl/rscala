...
s %@% '
  val c_10_3 = (1 to 3).map( i => (10-i+1) / i.toDouble ).product.toInt
'
@

This result is available in subsequent \Scala expressions by the identifier \code{c_10_3}
...
function.  Default values depend on the operating system so that 
...
If the result of method call on a \Scala reference is not a copyable type, then
...
\subsubsection{Length-one vectors as method arguments}
...
\R have special handling.  In the \R expression
...
the arguments) can be a literal or a variable.
...
element of the array using the following (where the last four statements are functionally equivalent).
