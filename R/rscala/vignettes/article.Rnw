...
The \pkg{rscala} package provides for string interpolation to evaluate
...
\emph{primitive types} supported by the \pkg{rscala} package. Further, \Scala
...
\Scala provides the \code{scalap}
executable to list the methods of classes and companion objects.  Its output is
...
reference can be copyable types (see Table \ref{mappings}) and \Scala
references.  One quirk worth noting is how length-one vectors in \R are
handled.  Note that in the previous \R expression \code{rng\$setSeed(24234L)},
the value \code{24234L} as an integer vector of length one in \R, but is passed
...
\subsubsection[Dealing with the Special apply and update Methods]{Dealing with the Special \code{apply} and \code{update} Methods}
...
\pkg{rscala} package, this works for \Scala snippets, but the \code{apply}
...
inner classes which are not supported.

If a method takes type parameters or is
...
automatically provides this support as well.
