...
\subsubsection{Arguments to Method Calls and Length-One Vectors}
...
Again, like for companion objects, each way has its advantages in terms of
...
automatically provides this support as well.  It may be helpful, for example,
...
\subsubsection[Assessing R from Embedded Scala]{Assessing \R from Embedded \Scala}
