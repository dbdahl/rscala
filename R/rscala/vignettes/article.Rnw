...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.5.0,\ \ Document ID: 2017-10-11}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
\code{s} in the package's namespace using the \code{scala} function with \code{mode="parallel"}.
