...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.4.1-SNAPSHOT,\ \ Document ID: 2017-10-06}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
and \code{scala3} are equivalent to \code{scala} except they change the default
arguments to return without blocking and assign the interpreter implicitly as a
promise.  There is no practical difference for \R scripts, but interative users
may notice that \code{scala2} feels faster.  Details on this and all
...
mind a few items.  If an argument to a \Scala function is not a copyable
type or a \Scala references, it will become a \code{EphemeralReference} object that
can still be used in \R code executed by the \Scala function.  (This functionality is
demonstrated later.)  If an argument
is not needed in the \Scala implementation (because, for example, it has
already been processed in the \R code), the conversion can be avoided by
deleting the object with \R's \code{rm} function before the \code{s \%!\%
'...'} snippet.  The automatically generated \Scala code for the conversion of
arguments can be displayed (for debugging purposes) by setting the
\code{show.header} option, e.g., \code{scalaSettings(s, show.header=TRUE)}.
The \code{scalaNull} function can be used to pass a null reference of a particular
type.  For example, an argument to a \Scala function might be \code{rng =
scalaNull("scala.util.Random")}, indicating a null reference to
...
copyable types, \Scala references, and \code{EphemeralReference}
objects, the last being automatically generated in \Scala functions for all arguments that are
wrapped by the \code{II} function or that are not copyable types and not \Scala references.
...
libraryDependencies += "org.ddahl" %% "rscala" % "2.4.1"
@
