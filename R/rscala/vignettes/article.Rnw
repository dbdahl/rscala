...
also use the older versions of Scala (e.g.,
...
order, the TCP/IP ports for the socket connections, and how output from \Scala
...
snippet of valid \Scala code that is evaluated in the embedded \Scala instance.
The \code{\%@\%} operator always returns \code{NULL}.  For example, consider
...
serializing output is often not needed yet requires some processing
overhead.
...
replaced with the string representation of the \R expression.  For example,
...
\code{Double} obtained by parsing a double literial obtained by string
...
\code{Boolean}, or \code{String} is passed to \R as a length-one vector of raw,
...
as vectors and matrices of the equivalent \R types.  We call values that are
primitives, arrays of primitives, and rectangular arrays of arrays of the
primitive types \emph{copyable} types.  The name is meant to remind us that
passing this kind of data over the interface between \Scala and \R involves
copying which, for a large data structure, can be computationally costly.
...
using example snippets assuming \code{row.major=TRUE} when calling the \code{scala}
function.
...
One of its methods is \code{range}.  The following three statements are all functionally equivalent:
...
flexibility, readability, and speed.  The first is pure \Scala code and
...
can be variables or literals.  The last is somewhat awkward, fast, and has the
...
operator.  For example, consider an array:
...
explicitly provide the type parameter.  When using the \code{\$} operator, the
