...
Note that \code{val} is a reserved word in \Scala and therefore our use here of
\code{val} and \code{.val} are guaranteed to not conflict with any variable names in \Scala.
...
flexibility, readability, and speed.  The first is mostly \Scala code and therefore
self-evident to a \Scala developer, but it is the slowest.
The second (using \code{s\$.}) is concise, fast, and flexible in that the arguments to the method
can be variables or literals.  The last method (using \code{s\$do}) is also fast but slightly awkward, however, it has
the added flexibility that the name
can be a literal (as in this example) or a variable.  Note that \code{do} is used here because it is a
reserved words in \Scala and is therefore guaranteed to not conflict with a variable name.
