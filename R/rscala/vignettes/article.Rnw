...
the first call because of caching the compilation of the \Scala snippet:
...
first is the \code{get} method that returns a \code{Tuple2} giving a value
...
convenient to use a suite of methods whose names start with \code{get} and end
...
\R expressions can also be evaluated with a suite of methods whose names start
with \code{eval} and end in $XY$, with $X$ and $Y$ having the same meaning as
