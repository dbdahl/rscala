...
Another example of a nontrival \Scala application that might benefit from
access to \R is \pkg{Apache Spark}, a cluster-computing framework for massive
datasets.  Although written in \Scala, \pkg{Spark} provides an applicaton
programming interface (API) to \Scala, \Java, \R, and \Python.  \R users who
are not already familar with \Scala would be best served by accessing
\pkg{Spark} through a dedicated \R package such as \pkg{sparklyr} or
\pkg{sparkr}.  Conversely, \Scala developers might prefer to interface directly
with \pkg{Spark}'s Machine Learning Library (MLlib) by writting \Scala code yet
supplementing MLlib with \R's analysis tools via a pool of \code{RClient}'s from \rscala.
