...
over the TCP/IP interface, a potentially-costly exercise for large data
...
compiler when appropriate, but must be explicit when using the
\code{\$} operator.
...
\Scala has a type parameterization which is similar but arguably more
...
\Rcpp and \pkg{inline} packages in \R for \C, \Cpp, and \Fortran.  We do not
