...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.3.1,\ \ Document Date: 2017-08-22}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
The goal of \rscala is to allow users to easily leverage the
unique strengths of \Scala and \R to make them more productive.
\Scala classes can be instantiated from \R and \Scala methods can be called.
Arbitrary \Scala code can be executed on-the-fly from within \R, inline \Scala
functions can be defined, and callbacks to \R are supported.  \R packages can
be developed using \Scala.  Conversely, \rscala also enables \R code to be
embedded within a \Scala application.  The \rscala package is available on CRAN
and has no dependencies beyond the \Scala standard library and base \R.
}
...
///After thinking it over, I believe the details of callbacks and SBT can be sacrificed in the intro paragraph to get at the overall idea.

This paper introduces \rscala, a software package that provides a bridge between \R and
\Scala.  The goal of \rscala is to allow users to leverage the
unique strengths of \Scala and \R in a single solution.  For example, \R packages can
implement computationally-intensive algorithms in \Scala, and conversely,
\Scala applications can take advantage of the vast array of statistical
libraries in \R. The \rscala package is available on the Comprehensive \R
Archive Network (CRAN) and its only dependencies are the \Scala standard
library and base \R.

///Does Spark need a citation?.

\Scala \citep{scala} is a mature, general-purpose programming language that has been used for enterprise-level
solutions such as \pkg{Spark}, the popular cluster-computing framework for
massive datasets.  Several books have been published recently on the utility
of \Scala in the areas of data science \citep{a9781785281372}, scientific
computing \citep{a9781785886942}, machine learning \citep{a9781783558742}, and
probabilistic programmings \citep{a9781617292330}.  Hence, we believe that \Scala
deserves consideration when looking for an effecient programming language
to complement \R.

///This next paragraphs are the most important in the paper. I suggest going into more depth to justify Scala. For example, the https://www.quora.com/How-does-Scala-compare-to-Java-8 points to an interesting conversation on the topic. There are several others, some more formal... Moreover, some short code examples or definitions could be helpful to explain features such as higher-order functions.

\Scala is designed to strike a balance between execution speed and programmer
productivity.  \Scala source code compiles to \Java bytecode and runs on the
Java Virtual Machine (JVM) at speeds comparable to \Java. \Scala code can be very concise,
thanks in part to type inference and higher-order functions. For
example...//////consider giving examples or explaining/// \Scala supports both
object-oriented and functional programming paradigms.///explain why this is
great/// Add more feature discussion here///

///This may seem agressive, but I removed the following paragraph. I believe the paper is cleaner and more interesting if you add more material about the Scala language and remove the more obvious discussion about R, Rcpp, and rJava. In comparison to Scala, the latter information is relatively well understood in the statistical computing community.

///Make sure citations for rJava and Rcpp and whatever else show up somewhere in the paper if this paragraph is removed. Actually, all of the citations need to be reviewed.

% \R \citep{r} is a scripting language and environment developed by statisticians
% for statistical computing and graphics.  \R has many contributors and a large
% base of statistically-oriented users.  \R's has over actively-maintained 11,000
% packages on CRAN (whereas \Scala's statistical libraries are not nearly as
% complete).  \R code can be very concise and expressive, but may also run
% significantly slower than compiled languages.  In fact, computationally
% intensive tasks in \R are typically implemented in \C, \Cpp, and \Fortran code.
% The manual ``Writing R Extensions'' details how to write \C, \Cpp, and \Fortran
% code that can be linked and called from \R at run time.  The \Rcpp package
% seeks to make it easier to integrate \C and \Cpp code in \R.  The \rJava
% package adds \Java as a fourth language that one might use to write \R
% extensions.  The \rscala package seeks to add \Scala as a fifth language for
% writing \R extensions.  That is, \rscala is to \Scala as \rJava is to \Java and
% as \Rcpp is the \C and \Cpp.  While the level of integration between \Scala and
% \R provided by \rscala is not as deep as those available to \C, \Cpp, and \Java
% provided by \Rcpp and \rJava, \rscala is sufficient in many cases.  For
% example, the \pkg{shallot} \citep{shallotSoftware} and \pkg{bamboo}
% \citep{bambooSoftware} packages on CRAN use \Scala via \rscala to implement
% statistical methodology of associated journal articles
% \citep{shallotPaper,bambooPaper}.  Beyond writing \rscala-based packages,
% \rscala also provides a simple and convenient way to leverage \Scala in an
% existing \R workflow.  Callbacks to \R from \Scala are supported which enables
% \Scala implementations of general algorithms (e.g., root finding methods) based
% on user-supplied \R functions.  In short, \Scala and \R each have distinct
% strengths and our \rscala software allows the user to easily leverage the
% advantages of each in a single project.
...
///insert a paragraph to pull R and Scala together using rscala. list features including inline functions, callbacks, arbitrary code exec, etc.

///In the following paragraph, I removed the parts relating to Java maintenance issues because I'm almost positive the editors will reject it based on their previous comments. Besides, these are closed issues and rJava continues to make progress. I'm not sure that these issues remain relevant.

The bridge between \R and \Scala is based on a custom TCP/IP
protocol.  Why use a custom protocol? 
Since \Scala code compiles to \Java byte code and runs on the JVM, it seems
natural that one could
access \Scala from \R via \rJava and then benefit from the speed of shared
memory.  In fact, an early implementation of (what is now known as) \rscala was
based on \rJava.  This implementation was abandoned for several reasons.
First, \rJava and \Scala's
read-eval-print loop (REPL) are both implemented using custom class loaders
which, in our experience, conflict with each other.  Second, since \rJava links to a single
instance of the JVM, one \rJava-based package can
configure the JVM in a manner that is not compatible with a second
\rJava-based package.  The current \rscala package creates a new instance of the 
JVM for each \Scala interpreter to avoid such conflicts. Finally, callbacks in \rJava are provided by the
optional \pkg{JRI} component which is only available if \R is built as a shared
library.  While this is the case on many platforms, it is not universal and
therefore callbacks could not be a guaranteed feature of \rscala software if
it were based on \rJava's \pkg{JRI}.

///this paragraph needs a leading sentence to connect it to the topic of the previous paragraph.
///for example, "It is also reasonable to suggest that \rscala build on packages Rserve or RCaller... 
///This paragraph mentions RCaller, but does not address it.

The ability of \rscala to embed \R in a \Scala application is an offshoot the
callback functionality.  Since \Scala can call
\Java libraries, those who are interested in accessing \R from \Scala should
also consider the \Java libraries \Rserve
\citep{rserve} and \pkg{RCaller} \citep{rcaller}.  Like \rscala, \Rserve is ``a
...
\rscala.  Unfortunately, like \rJava, \Rserve requires that \R be compiled as a
shared library.  \Rserve also has some limitations on Windows and the user is
advised to not ``use Windows unless you really have to''
(\url{http://www.rforge.net/Rserve/doc.html}).

The paper is organized as follows.  Section \ref{scalainr} describes
using \Scala from \R.  Some of the more important topics
presented there include the data types supported by \rscala, embedding \Scala
snippets in an \R script, accessing precompiled \Scala code from \R, and calling
back into \R from embedded \Scala.
We also discuss how to develop \R packages based on \Scala and refer readers
to two example packages.
Section \ref{rinscala} describes using \R from
\Scala.
Section \ref{bootstrap} provides an case study to show how
\Scala can easily be embedded in \R to significantly reduce computation
time for a simulation study.  We conclude with future work in Section
\ref{conclusion}.
...
Debian/Ubuntu based systems), or users can install \Scala manually as described
on the \Scala webpage.
...
binomial coefficient ${10 \choose 3} = \prod_{i=1}^3 (10-i+1)/i$.  The code
below uses \Scala's \code{val} statement to define an immutable variable
\code{c_10_3} by computing the \code{product} of applications of the
higher-order \code{map} function to a range \code{1 to 3}.
...
in the classpath.  The last method (using \code{s\$do}) is also fast and
...
evaluation in the \R session.  This feature is inspired by the \Rcpp and
\pkg{inline} \R packages for \C, \Cpp, and \Fortran.  We do not recommend that
...
\code{s} is the \Scala instance and \code{'...'} represents the multiline
\Scala snippet.  The effect of the \code{\%!\%} operator is two-fold: i.~it
automatically makes the arguments of the enclosing function (namely, \code{n}
and \code{format}) available in the \Scala snippet, and ii.~it caches the
on-the-fly compilation of the \Scala snippet to substantially improve speed for
subsequent calls to \code{bell.version2}.  The \code{\%!\%} operator returns a
copyable type or a \Scala reference if the result is not a copyable type.  The
\code{\%.!\%} operator is identical except that it always returns a \Scala
reference.

The consumer of a \Scala function does not need to know that the implementation
is written in \Scala.  There are two considerations, however, for the
programmer of a \Scala function.  First, the arguments to \Scala functions can
be copyable types, \Scala references, or \code{NULL}, where \code{NULL} is used
to indicate arbitrary \R objects that will not be converted to \Scala values
but will still be used in \R code.  Second, a default value must be provided
for every argument, thereby indicating the argument's type to \rscala.  If the
user passes arguments of different types than those indicated by the default
values, they are cast to the expected types.  The \code{scalaNull} function can
be used to specify an argument's \Scala type without having to instantiate an
object of that class.  For example, an argument to a \Scala function might be
\code{rng = scalaNull("scala.util.Random")}, indicating that the argument
should be of type \code{scala.util.Random}.
...
literals and variables of copyable types, \Scala references, and \code{NULL},
where \code{NULL} is used to indicate arbitrary \R objects.
...
argument, \code{func}, takes a user-defined \R function.  The argument's type
is set as \code{NULL} so its value can be referenced in \Scala without any
conversion.
...
characteristics of \rscala itself.
...
the first time the function is run.  Subsequent invocations are faster because
...
\code{s} in the package's namespace using \R's \code{delayedAssign} function.
This ensures that the start-up cost of instantiating a \Scala instance is only
incurred when \code{s} is first used.  At that time, if \Scala itself is not
...
developers are encouraged to cross-compile for releases 2.10, 2.11, and 2.12
and to place compatible \Scala JAR files in \code{inst/java/scala-2.10},
...
import the Apache Commons Mathematics Library \citep{commonsMathSoftware} JAR
files distributed with the \pkg{commonsMath} package.  The rational is that JAR
...
Figure \ref{screenshot} shows a screen shot of the application.
...
While the previously mentioned \pkg{shallot} and \pkg{bamboo} demonstrate the
ability to develop packages based on \rscala, we demonstrate in this section
the easy with which computationally-intensive statistical procedures can be
...
By way of comparison, we implement the same algorithm using pure \R and using
inline \Cpp code via the \Rcpp package.  All four implementations provide a
function taking the same arguments, including a user-specified \R
function for sampling.

We consider a
simulation study of the coverage of a bootstrap confidence interval procedure.
Consider a population parameter $\beta_1/\beta_2$, where $\beta_1$ and
$\beta_2$ are population quantiles associated with probabilities $p_1$ and
$p_2$, respectively.  Based on a sample of $n$ observations, a point estimator
of the parameter is the ratio of the corresponding sample quantiles and the
following bootstrap procedure can be used when the population distribution is
...
user-supplied sampling function.  Although the code is general, we sample $n=100$
observations from the standard normal distribution and set $p_1=0.75$ and
$p_2=0.35$, making $\beta_1/\beta_2 \approx -1.75$.
We use \code{nIntervals} = 10,000 Monte Carlo replicates, each having
\code{nSamples} = 10,000 bootstrap samples.

The four implementations of the algorithm are contained in the \R script in the
Appendix.  The \R implementation is the shortest and the \rscala implementation
is somewhat more concise than the \Rcpp implementation.  The pure \R
implementation loops using apply functions.  The \Rcpp implementation is
written in a \C style.  The pure \R, \Rcpp, and second \rscala implementations
...
sampling the data.  On machines with many cores, each thread having to wait its
...
3.4.1, \Scala 2.12.3, \Java 8, \Rcpp 0.12.12, and a prerelease version of \rscala
2.3.1.
...
Executions times (in seconds) for 10 replications of the simulation study are
...
\rscala implementation (which uses the \pkg{parallel} package) was the overall
the fastest.  The first \rscala implementation was close behind, expect on the
Ubuntu machine with 56 cores, which illustrates the difficulty of sharing an
single \R instances for all the cores.  The \Rcpp implementation is generally
slower than the \rscala implementations, but within the same order of magnitude
relative to the pure \R implementation.  We are not experts in \Cpp and it is
possible the \Rcpp implementation could be improved.


\section{Conclusion}
\label{conclusion}

///Revise the intro sentence. Uses "allow" twice and has several grammatical issues. Consider breaking it up.

This paper introduced the \rscala package that allows users to bridge \R and
\Scala in one program or package, allowing a user to leverage their skills in
both languages and to utilized libraries and exploit strengthens in each
language.  We are exploring two possible features to improve the package.  The
first is to allow embedded \Scala computations to be interrupted by the \R user
without destroying the TCP/IP bridge.  The second feature is allowing \R and
\Scala to run on separate machines.

\section*{Acknowledgements}

The author's work on this paper was supported by NIH NIGMS R01 GM104972.  The
author thanks the CRAN maintainers for the patience and excellent service.  The
author also thanks the following students for valuable feedback on the software
and paper: Floid Gilbert, Brandon Carter, Deepthi Uppalapati, Scott Ferguson,
and Richard Payne.
...
{
\footnotesize
\VerbatimInput[numbers=left,xleftmargin=5mm]{bootstrap-coverage.R}
}
