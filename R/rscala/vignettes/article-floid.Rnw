...
\title{\textit{\Large Under Review\\\footnotesize Software Version: 2.2.3,\ \ Document Date: 2017-06-10}\\\vspace{5ex} Integration of \R and \Scala Using \rscala}
...
installation, elevated privileges are not required.
...
By default, the \code{scala} function searches your system for a suitable
...
Notice that the side effect of printing \code{120} to the
console is still displayed even though the operator returns \code{NULL}.

\Scala values defined outside of functions or methods are available in subsequent \Scala
expressions. For example, the value \code{c_10_3} defined in the previous code snippet
still exists in the \Scala environment, as shown here.
...
operator which returns the result of the last expression in the snippet to \R.

<<evaluateScalaCodeSimple2>>=
result <- s %~% '(1 to 3).foldLeft(1) { (prod,i) => prod * (10-i+1)/i }'
result
@
...
the \R code's return value before the \Scala snippet is executed. The \R code is executed in the same environment (i.e., scope) as
...
example, that second expression below is false. This is because \code{@\{nThreesomes\^{}20\}} is replaced by a string literal
(i.e., ``6.191736e+20''), which is slightly different than the calculated value.
...
If names of \Scala methods are not valid identifiers in \R, quote them to
avoid parsing errors in R.  For example, the method \code{:+} is quoted here:
...
\todo{Introduce the three ways of calling methods independently of companion objects.
Note how to use companion objects at the end of this same section, or in a small section immediately following.}
...
regularly-spaced elements.  The following statements are all functionally
equivalent.
...
These three expressions each have their advantages in terms of
flexibility, readability, and speed.  The first expression uses \Scala code and
is therefore natural for a \Scala developer, but it is the slowest to execute.
The second expression is both fast and concise. It is also flexible in that the method arguments
can be variables or literals.  The last expression is somewhat awkward, but it is fast and
the name of the companion object can be specified using either a variable or a literal.

The result of the previous example, \code{oneToTen}, is an \R vector, because \code{range} returns a copyable type.
If a \Scala reference is desired instead, the expressions can be modified as follows:
...
\Scala provides an executable named \code{scalap} to list the methods of classes and
companion objects.  The \rscala package provides the same functionality in \R with the
...
\code{rng}), use the \code{scalap} function as follows.

<<scalap2, out.lines=10>>=
scalap(rng)
@

\todo{also mention discovering methods using \$ with auto-complete}

\subsubsection[The apply and update methods]{The \code{apply} and \code{update} methods}

In \Scala, if \code{a} is an object, then the expression \code{a(x)} is
shorthand for \code{a.apply(x)}. Similarly, \code{a(x) = y} is shorthand for
\code{a.update(x, y)}.  The \rscala package supports \Scala's shorthand notation in code
snippets, but \code{apply} and \code{update} must be specified explicitly when
using the \code{\$} operator.

The following example creates an array containing a Fibonacci sequence. The
first line demonstrates \Scala's shorthand notation for \code{apply} in
a code snippet. The second line is equivalent, but explicitly specifies the
method \code{apply}. The third and fourth lines yield identical results via
the \code{$} operator.
...
In this example, we demonstrate the \code{update} method. First, we create an
array in \Scala, and then we assign the value $\pi$ to the second element of
the array using four different, but equivalent, expressions.
...
\todo{Probably remove this since it will be addressed directly in "Calling Scala methods"}
The previous example also illustrates that the ``\code{s\$.}'' and
``\code{s\$do}'' notations --- which were first introduced for companion
objects --- can also be used for existing \Scala values. In this case, we referenced
the value \code{fibSeq}.

\subsubsection{Instantiating objects}

Because methods can be called in three ways, it follows that creating new objects (i.e., calling constructor methods) can also be done in the same three ways. This example illustrates each approach by creating new instances of \code{scala.util.Random} and returning a \Scala reference to \R.
...
reference can be copyable types and \Scala references.  The \rscala package provides special
handling for \R vectors of length one. In the previous \R expression,
...
This is usually the desired behavior in common use.  However, if a length-one \R
vector should be treated as an array in \Scala,
wrap the vector in a call to the \code{I} function to ensure
...
wrapped by the \code{I} function.  In the example below, the first argument to \code{apply} is
...
Because \Scala supports using
\Java classes, the \rscala package
automatically provides this support as well. The example below
...
The \rscala package reduces the complex task of data exchange between \R and
\Scala to a single line of code.
\Scala values can be obtained using the \code{\$} operator and the name of
the identifier as in this example. The syntax of the third expression ensures that a \Scala
reference is returned.

\todo{Where does "val" come from? Is it a Scala function?}
...
Values of copyable types and \Scala references are set in the
\Scala session using assignment with the \code{\$} operator:
...
of $n$ items (i.e., the Bell number \citep{bell}).  This
...
an algorithm based on the Bell triangle (also
...
for $n \le 22$, \emph{approximate} answers for $22 < n \le 218$, and
...
still occurs in \R when \code{format="integer"} or \code{format="double"} for small
$n$, but there is no overflow for almost any $n$ when \code{format="log"},
...
reference if the result is not a copyable type.  Its companion operator,
\code{\%.!\%}, is identical except that it always returns a \Scala reference.
...
programmer of a \Scala function.  First, the terminating expression of a \Scala
...
indicate arbitrary \R objects that will not be converted to \Scala values.  Third, default values must be provided for
...
instance.  A link to the comprehensive Scaladoc for the \code{RClient} class is located on GitHub:
...
first method is \code{get}. It returns a typle of two elements where the first member
is the \R variable's value (statically typed as \code{Any}), and the second member is the resulting \Scala type name (a \code{String}).
...
to return the value of the \R variable \code{T} as a logical scalar value.
...
<<rootFinding>>=
bisection <- function(func=NULL, lower=1.0, upper=1.0, epsilon=0.000000001)
  s %!% '
...
As such, the user does not often need to think about memory management
issues. There are a few important things to note, however.  First, the default
...
implement the statistical methodology related to their associated journal articles
...
Apache Commons Mathematics Library JAR files distributed with the \pkg{commonsMath} package.  The rational is
...
provides \Scala expressions that should be evaluated when \Scala is
...
also allows an \R interpreter to be embedded in a \Scala application via the
\code{org.ddahl.rscala.RClient} class.
As a convenience, \rscala's JAR files are
...
Note that the \rscala package does \emph{not} need to be installed in \R
...
the \R installation on Windows.  On other operating systems, \code{R} is assumed to be
...
produce plots in a \Scala application.  This example illustrates all four tasks: i.~it finds the city in Europe that is closest on
average to all other European cites (as given by \R's builtin dataset
\code{eurodist}), ii.~it wraps \R's \code{qnorm} function to define a method in
\Scala by the same name, iii.~it writes data to disk and reads it again, and
iv.~it produces a dendrogram for hierarchical clustering as in Figure \ref{fig:dendrogram}.
...
The \code{scala} function starts a \Scala interpreter using R's
...
execute \Scala code on-the-fly as well as set/get values of
...
server.  There are two versions of the \code{RClient} class. The \Java
...
\clearpage
[Calling into the JVM and back into R]{Calling into the JVM and back into \R}
...
[Calling into R from a JVM application]{Calling into \R from a JVM application}
\label{benchmarksJVM}
