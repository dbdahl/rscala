...
The \code{scala} function uses \R's
\code{system2} function to start the Java Virtual Machine running a TCP/IP server
using a custom protocol and whose port is randomly selected by default.
The \Scala server has an instance of
\code{scala.tools.nsc.interpreter.IMain}, the class representing \Scala's
read-eval-print-loop (REPL).

  The server accepts only a single local connection.  While the
the server is starting up, \R repeatedly tries to connect using
