...
\subsubsection{Evaluation of \Scala snippets}
...
Notice the side effect of printing \code{120} to the
console even though the operator returns \code{NULL}.
The console printing behavior depends on the
environment and operating system in which R is run.  The arguments
