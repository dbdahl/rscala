...
\code{system2} function to start the Java Virtual Machine running code that implements a TCP/IP server
...
given value.  The server accepts only a single local connection.  While the \Scala
server is starting up, the \R component repeatedly attempts to establish a connection using
