...
functions can be defined, and callbacks to \R are supported.  \R packages can be
developed using \Scala.  Conversely, \rscala also enables \R code to be
...
This paper introduces \rscala, software that provides a bridge between \R and
\Scala.  The goal of \rscala is to allow users to leverage the unique strengths
of \Scala and \R in a single program.  For example, \R packages can implement
computationally-intensive algorithms in \Scala, and conversely, \Scala
applications can take advantage of the vast array of statistical libraries in
\R.  Callbacks from embedded \Scala into \R are supported.  The \rscala package
is available on the Comprehensive \R Archive Network (CRAN) and its only
dependencies are the \Scala standard library and base \R.  Conversely, \R can
be embedded within a \Scala application by a one-line dependency declaration in
\pkg{Scala Build Tool} (SBT).

\Scala \citep{scala} is a general-purpose programming, statically-typed language that has been used for enterprise-level
solutions such as \pkg{Spark} (CITATION), the popular cluster-computing framework for
...
deserves consideration when looking for an efficient, general-purpose programming language
to complement \R.
...
\Scala.  In both Sections\ \ref{scalainr} and \ref{rinscala}, concise examples are provided to help
describe the software's functionality.
...
utilized libraries and exploit strengthens in each language.  For example, \R
users can implement computationally intensive algorithms in \Scala, write \R
packages based on \Scala, and can access \Scala libraries from \R.  \Scala
programmers can take advantage of \R's tools for data analysis and graphics
from within a \Scala application. 

We are exploring two possible features to improve the package.  The first is to
allow embedded \Scala computations to be interrupted by the \R user without
destroying the TCP/IP bridge.  The second feature is allowing \R and \Scala to
